...
def guia_de_uso() -> str:
    return (
        ...
        "4. **`impo_buscar_texto`** — búsqueda full-text REAL del sitio vía el feed "
        "RSS de WordPress (`/?s=...&feed=rss2`). Devolvé coincidencias "
        "estructuradas (titulo, url, fecha, autor, categorias) con paginación. "
        "Usala cuando hay un tema pero no tipo/número/año.\n"
        "5. **`impo_novedades`** — últimas novedades y publicaciones editoriales "
        "(incluye anuncios de nuevas leyes) desde el feed `/feed/`.\n"
        "6. **`impo_referencias_norma`** — grafo de referencias cruzadas de una "
        "norma (reglamentada/modificada/ver) a partir de `referenciasNorma`.\n\n"
        ...
        "- **Búsqueda**: `impo_buscar_normativa` es best-effort (resuelve a la "
        "norma o degrada a URLs). Para búsqueda full-text real usá "
        "`impo_buscar_texto`, que consume el feed RSS de WordPress "
        "(`/?s=...&feed=rss2`, **UTF-8**, no latin-1). El WP REST API "
        "(`/wp-json/`) está bloqueado (403).\n"
